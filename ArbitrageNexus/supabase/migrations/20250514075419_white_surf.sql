/*
  ...
  1. New Tables
    ...
    - `api_key_audit_logs`
      - `id` (uuid, primary key)
      - `api_key_id` (uuid, references exchange_api_keys)
      - `action` (text)
      - `ip_address` (inet)
      - `user_agent` (text)
      - `details` (jsonb)
...
-- Create unique constraint to prevent duplicate exchange entries per user
CREATE UNIQUE INDEX exchange_api_keys_user_id_exchange_name_key 
ON exchange_api_keys(user_id, exchange_name);
